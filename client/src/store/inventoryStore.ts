   ...
   persist(
      (set, get) => ({
         ...
         fetchProducts: async (_force = false) => {
            const state = get();
            if (state.isInitialized && !_force && state.allProducts.length > 0) {
               get().applyFilters();
               return;
            }
         ...
         applyFilters: () => {
            ...
            if (search.trim()) {
               ...
               filtered = filtered.filter(
                  p => normalize(p.description).includes(term) || (p.sku && p.sku.includes(term)),
               );
            }
         ...
         setSort: key => {
            set(state => {
               ...
               const newDirection =
                  isSameKey && state.sortConfig.direction === 'asc' ? 'desc' : 'asc';
